...
/**
 * ConvertAudioDialog (فاز A2 — RA2.4/RA2.5): the Persian gate for audio
 * files the browser cannot play (or the always-convert `.wav`/`.flac`
 * set — Appendix A-1).
 *
 * «تبدیل به MP3 (آفلاین)» runs the ffmpeg.wasm conversion ON THE SHARED
 * ENGINE (bundled, lazy-loaded, 100% offline — A.2.5; the video
 * converter's exact instance, never a second core) with a live progress
 * bar and a WORKING cancel: aborting terminates the engine, cleans the
 * temp FS state and creates NOTHING (RA2.5 — no object, no residue).
 * The ORIGINAL file is kept in the sidecar next to the converted MP3
 * (`origAssetHash`); the object points at the converted asset while
...
import { importConvertedAudioFile } from "@/ui/clipboard/audioImport";
import { formatInteger } from "@/ui/i18n/numbers";

/** The audio convert-gate dialog (visible while the queue is non-empty). */
export default function ConvertAudioDialog(): ReactNode {
  const { t, language } = useTranslation();
  const queue = useUiStore((state) => state.pendingAudioConversions);
  const clearQueue = useUiStore((state) => state.clearAudioConversions);
  ...
  /** Cancels: terminate the shared engine, clean temp state, queue empties. */
  ...
  const convert = useCallback((): void => {
    ...
    void Application.boot().then((context) => {
      ...
      const runQueue = async (): Promise<void> => {
        while (true) {
          const head = useUiStore.getState().pendingAudioConversions[0];
          if (head === undefined || abort.signal.aborted) {
            break;
          }
          setProgress(0);
          const converted = await audioConverter.convert({
            file: head.file,
          ...
          const at =
            head.at !== undefined
              ...
              : undefined;
          await importConvertedAudioFile(
            services,
            head.file,
            converted,
            rect,
          ...
          useUiStore.getState().shiftAudioConversion();
        }
        setBusy(false);
      };
      void runQueue();
    ...
    <Dialog open={open} onOpenChange={(next) => {
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>{t("audioConvert.description")}</DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1.5" role="status" aria-live="polite">
              <div className="flex items-center justify-between text-[11px] text-muted-foreground">
                <span className="flex items-center gap-1.5">
                  ...
                  {t("audioConvert.converting")} — {headName}
                </span>
          ...
          {failed ? (
            <p className="text-xs text-destructive">{t("audioConvert.failed")}</p>
          ) : null}
        </div>
        ...
        <DialogFooter className="gap-2">
          {busy ? (
            <Button type="button" variant="ghost" onClick={cancel}>
              {t("audioConvert.cancelRun")}
            </Button>
          ) : (
            <Button type="button" variant="ghost" onClick={cancel}>
              {t("audioConvert.cancel")}
            </Button>
          )}
          <Button
            type="button"
